...
const REVIEWS = {
  ...
  /* Main load — tries cache → Firebase → SerpAPI → static */
  async load(onReady) {
    // 1. localStorage cache — show immediately
    try {
      const c = JSON.parse(localStorage.getItem(this.CACHE_KEY) || 'null');
      if (c?.reviews?.length) {
        onReady(this.sort(c.reviews));
        if (Date.now() - c.ts < this.CACHE_TTL) return; // fresh enough
    ...
    // 2. Firebase — fast persistent store
    try {
    ...
    // 3. Static fallback while fetching
    onReady(this.sort(CONFIG.staticReviews || []));

    // 4. SerpAPI live fetch
    this._refresh(onReady);
  },
